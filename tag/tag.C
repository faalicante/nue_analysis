...
void getEntriesInEllipse(TH2F &h2, TEllipse &el, int *entries) {
...
void makePlots(int cell, TCanvas *c, int np, int npmax, TH1F *h_long, int *maxPeak, int *maxPlate) {
  ...
  *maxPlate = h_long->GetMaximumBin();
  ...
  h_long->Draw("hist");
  if(np == 2)                 c->Print(Form("%s/longitudinal_xz_%i.pdf(", path.Data(), cell), "pdf");
  else if(np == npmax-1)      c->Print(Form("%s/longitudinal_xz_%i.pdf)", path.Data(), cell), "pdf");
  else if(idx == 3 && np >2 ) c->Print(Form("%s/longitudinal_xz_%i.pdf", path.Data(), cell), "pdf");
}
...
void makeNtuple(int cell, TH1F **h_long, TObjArray &peaks, int *ranks) {
  TString outputFileName = TString::Format("%s/peaks_%i.root", path.Data(), cell);
...
int main(int argc, char* argv[]) {
  ...
  TString file =  TString::Format("%s/hist_XYP_b121_%i.root", path.Data(), cell);
  ...
  h2->Draw("colz");
  c->Update();
  c->Print(Form("%s/sh_%i.gif+180", path.Data(), cell));
  TObjArray peaks;
  TObjArray txt;
  int ranks[ntag];
  ...
  for(int p=1; p<=nPlates; p++) { 
    printf("Plate %i\n", p);
    TH2F *h = (TH2F *)(f->Get(Form("XYseg_%i",p)));
    // h->Smooth();
  ...
  c->Print(Form("%s/sh_%i.gif++", path.Data(), cell));
