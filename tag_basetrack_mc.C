...
void makePlots(int brick, TCanvas *c, int np, int npmax, TH1F* h_long, int *maxPeak, int *maxPlate) {
  TString path = Form("/eos/user/f/falicant/Simulations_sndlhc/nuecc_withcrisfiles_25_July_2022/b0000%i",brick);
  int idx = (np%3) +1;
  if (idx==1) c->Clear("D");
...
void tag_basetrack_mc(int brick) {
  TString path = Form("/eos/user/f/falicant/Simulations_sndlhc/nuecc_withcrisfiles_25_July_2022/b0000%i",brick);
  int ntag = 50;
  TH2F *h2 = (TH2F *)(gDirectory->Get("XYseg"));
  // TH3F *h3 = (TH3F *)(gDirectory->Get("XYPseg"));
  h2->Rebin2D(2,2);
  // h2->GetXaxis()->SetRangeUser(320000,360000);
  // h2->GetYaxis()->SetRangeUser(20000,60000);
  h2->Smooth();
  // set_limits(*h2, *h2);
